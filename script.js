// Three.js Scene Setup with Star Wars Opening Crawl
...
function createStarField() {
    ...
    for(let i = 0; i < starsCount; i++) {
        // Random distribution across entire space
    ...
    for(let i = 0; i < brightCount; i++) {
        ...
        brightPositions[i * 3 + 1] = (Math.random() - 0.5) * 150;
        brightPositions[i * 3 + 2] = (Math.random() - 0.5) * 100 - 20;
        
        brightSizes[i] = Math.random() * 3 + 1;
    }
    ...
    // Individual floating velocities
    for(let i = 0; i < brightCount; i++) {
        brightStars.userData.velocities.push({
...
function animate() {
    ...
    scene.children.forEach(child => {
        // BRIGHT FLOATING STARS
        if(child.userData.type === 'brightStars') {
            ...
            for(let i = 0; i < positions.length; i += 3) {
                ...
                // Wrap around
                if(Math.abs(positions[i]) > 75) velocities[index].x *= -1;
                if(Math.abs(positions[i + 1]) > 75) velocities[index].y *= -1;
                if(Math.abs(positions[i + 2]) > 50) velocities[index].z *= -1;
            }
...
// the third Animated Script starwars theme not true
